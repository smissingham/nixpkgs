...
buildNpmPackage {
  ...
  buildPhase = ''
    ...
    # Verify the bundled rclone matches Filen's expected version.
    grep -Fxq 'export const RCLONE_VERSION = "${rclone.version}"' src/lib/rclone/constants.ts || {
      echo "nixpkgs rclone ${rclone.version} does not match Filen's expected rclone version" >&2
      exit 1
    }
